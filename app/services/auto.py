    ...
    def parse_page(self) -> int:
        try:
            ...
            while True:
                ...
                if len(href) != 0:
                    price = self.driver.find_elements(by=By.XPATH, value='//a[contains(@class, "ListingItemPriceNew__link")]')
                    ...
                    for i in range(len(ad_href)):
                        ...
                        specifications_urls = self.driver.find_elements(by=By.XPATH, value='//a[contains(text(),"Характеристики модели в каталоге")]')
                        specifications_urls = list(map(lambda x: x.get_attribute('href'), specifications_urls))
                        if len(specifications_urls) != 0:
                            print(len(specifications_urls))
                            self.driver.get(url=specifications_urls[0])

                            specifications = self.driver.find_elements(by=By.XPATH, value='//dl[contains(@class,"list-values clearfix")]')
                            specifications_dict_list = list(map(lambda x: self.parse_specs(x.text), specifications))
                            for specifications_dict in specifications_dict_list:
                                self.advertisement[ad_href[i]] = self.advertisement[ad_href[i]] | specifications_dict
                    ...
                    df.to_csv(f'auto{page}.csv', index=False)
